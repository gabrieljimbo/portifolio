...
**Check for `product-brief.md` in the project root.** If found, use it for audience, product, and brand context -- skip questions that the brief already answers. If not found, auto-scan the project to build context: read `CLAUDE.md`, `README.md`, and glob for existing copy/marketing files (`**/*copy*`, `**/*deck*`, `**/marketing/**`). Present what you found and ask the user to confirm and fill gaps before writing.
...
- **humanizer**: For removing AI writing patterns from generated copy
- **copy-reviewer**: For a senior creative director audit of finished copy
- **pipeline**: To run the full copy deck quality pipeline
